    ...
    with st.form("config"):
        ...
        if st.form_submit_button("submit"):
            if yturl:
                ...
                transcripts, sentences = get_transcripts.get_transcripts_from_url(
                    audio_file_url=yturl
                )
                content_title, summary, Notes, questions = (
                    get_content_summary.generate_summary_studyplan_QandA_Gemini(
                        transcripts
                    )
                )
...
title = st.write(f"{content_title}")
